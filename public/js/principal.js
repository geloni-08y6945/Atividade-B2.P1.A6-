...
async function buscarPrevisaoDetalhada(cidade) {
    ...
    const backendUrl = `http://localhost:3001/api/previsao/${encodedCidade}`;
...
function updateDisplayContent(veiculo) {
    ...
    todosBotoesAcao.forEach(btn => hideElement(btn));
    ...
    if (!isBike) {
        ...
        if (!(isEsportivo && veiculo.turboAtivado)) {
